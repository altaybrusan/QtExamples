...
void MainWindow::createLayout()
{
    ...
    statusBar->buttonBox()->addButton(generateOrCancelButton, QDialogButtonBox::ActionRole);
    statusBar->buttonBox()->addButton(quitButton, QDialogButtonBox::AcceptRole);
...
{
    connect(firstButton, SIGNAL(clicked()),this, SLOT(setFirstImage()));
    connect(lastButton, SIGNAL(clicked()), this, SLOT(setLastImage()));
    connect(generateOrCancelButton, SIGNAL(clicked()),this, SLOT(generateOrCancelImages()));
...
void MainWindow::setImageFile(QLabel *targetLabel,const QString &which)
{
...
void MainWindow::createAndRunACrossFader(int number, const QImage &firstImage,
                                         const QImage &lastImage)
{
    ...
    QGridLayout *layout = qobject_cast<QGridLayout*>( progressWidget->layout());
    Q_ASSERT(layout);
    layout->addWidget(progressLabel, number, 0);
    layout->addWidget(progressBar, number, 1);

    double firstWeight = (number + 1) /  static_cast<double>(numberSpinBox->value() + 1);
    double secondWeight = 1.0 - firstWeight;
    CrossFader *crossFader = new CrossFader(filename, firstImage, firstWeight, lastImage, secondWeight, this);
    crossFaders << crossFader;

    connect(crossFader, SIGNAL(progress(int)), progressBar, SLOT(setValue(int)));
    connect(crossFader, SIGNAL(saving(const QString&)),this, SLOT(saving(const QString&)));
    connect(crossFader, SIGNAL(saved(bool, const QString&)), this, SLOT(saved(bool, const QString&)));
    connect(crossFader, SIGNAL(finished()), this, SLOT(finished()));
    crossFader->start();
}
